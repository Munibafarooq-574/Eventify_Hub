...
const VendorDetailsScreen: React.FC = () => {
  const { id, packageId, openTab } = useGlobalSearchParams();
 ...
 useEffect(() => {
  ...
  } else if (typeof openTab === 'string' && openTab === 'Packages') {
    setActiveTab('Packages');
  }
}, [packageId, openTab]);

  const getSelectedPackage = () => {
    if (!vendorData?.packages?.length) return null;

    if (activePackage) {
      const selected = vendorData.packages.find(
  ...
  const handlePackageSelect = (packageIdValue: string) => {
    setActivePackage(packageIdValue);
  };
  ...
  const handlePickMedia = async () => {
    ...
    try {
      ...
      const uploadedMedia: ReviewMedia[] = uploadedUrls.map(
        (url, index) => ({
          type:
  ...
  const fetchReviewSummary = async () => {
    ...
    try {
      const data = await getVendorReviewSummary(
        vendorData._id,
      );
    ...
    try {
      ...
      const vendorIndex =
        cart.vendors.findIndex(
          (vendor: any) =>
            vendor.vendor?._id === vendorData?._id,
        );

      if (vendorIndex !== -1) {
        const packageExists =
          cart.vendors[vendorIndex].packages?.some(
            (existingPkg: any) =>
              String(existingPkg._id) ===
              String(pkg._id),
          );

        if (!packageExists) {
          cart.vendors[vendorIndex].packages.push(
            pkg,
          );
        }
      } else {
  ...
  useEffect(() => {
    const fetchVendorDetails = async () => {
      try {
        ...
        setActivePackage(validRoutePackage);
      } catch (error) {
        console.error(
    ...
    return (
      ...
      >
        ...
        <Text style={styles.errorText}>
          Failed to load vendor details. Please try
          again.
        </Text>
      </View>
    );
    ...
    >
      ...
      >
        ...
        {activeTab === 'Packages' && (
          <View style={styles.detailsContainer}>
            <Text
              style={styles.packagesSectionTitle}
            >
              Available Packages
            </Text>
            ...
            vendorData.packages.length === 0 ? (
              ...
              >
                ...
                <Text
                  style={styles.noPackagesTitle}
                >
                  No Packages Available
                </Text>

                <Text
                  style={styles.noPackagesText}
                >
                  This vendor has not added any
                  packages yet.
                </Text>
              </View>
            ) : (
        ...
        {activeTab === 'Reviews' && (
          <View style={styles.tabContent}>
            ...
            <View
              style={styles.reviewControls}
            >
              <Text
                style={styles.reviewControlTitle}
              >
                Filter Reviews
              </Text>
