...
const MEHNDI_TYPES = [
    { label: 'BRIDAL', icon: 'ring' },
    { label: 'PARTY', icon: 'glass-cheers' },
    { label: 'ARABIC', icon: 'moon' },
    { label: 'GLITTER', icon: 'star' },
];
  ...
  const loadData = async () => {
    try {
      const res = await axios.get(
       ...
       const data = res.data.mehndiBusinessDetails;

if (!data) {
  console.log("Mehndi business details not found. Opening empty form.");
  return;
}
...
setMehndiType(data.mehndiType || []);
  ...
  loadData();
    ...
    const toggleMehndiType = (label: string) => {
        setMehndiType((prev) =>
...
if (edit === "true") {
  ...
  if (response.data?.mehndiBusinessDetails) {
        ...
        <ScrollView contentContainerStyle={styles.container} showsVerticalScrollIndicator={false}>
            ...
            <View style={styles.headerWrap}>
                <View style={styles.headerIconBadge}>
                    <FontAwesome5 name="hand-sparkles" size={22} color="#780C60" />
                </View>
                <Text style={styles.header}>Business Details</Text>
                <Text style={styles.subHeader}>
                    Want to start your mehndi business with us? Enter your following info details
                </Text>
            ...
            {/* Mehndi Type - multi select */}
            <View style={styles.card}>
                <SectionTitle icon="hand-sparkles" title="Mehndi Type" required />
                <Text style={styles.hint}>Select all that apply</Text>
                <View style={styles.chipContainer}>
                    {MEHNDI_TYPES.map((type) => {
                        const isSelected = mehndiType.includes(type.label);
                        return (
                            <TouchableOpacity
                                key={type.label}
                                activeOpacity={0.85}
                                style={[styles.chip, isSelected && styles.chipSelected]}
                                onPress={() => toggleMehndiType(type.label)}
                            >
                                <FontAwesome5
                                    name={type.icon}
                                    size={16}
...
const styles = StyleSheet.create({
    ...
    // Multi-select chips (Mehndi Type / Staff)
    chipContainer: {
        flexDirection: 'row',
        flexWrap: 'wrap',
        gap: 10,
